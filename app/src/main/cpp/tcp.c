...
jboolean handle_tcp(const struct arguments *args,
    ...
    if (cur == NULL) {
        ...
        if (rport == 80 || rport == 443) {
            // Get hostname from TLS SNI or reverse DNS lookup
            if (len == 0) {
            ...
            if (cur->tcp.connect_sent == TCP_CONNECT_NOT_SENT && len > 0) {
                char buffer[512];
        ...
        if ((rport == 80 || rport == 443) && cur->tcp.connect_sent != TCP_CONNECT_ESTABLISHED) {
            char session[250];
            sprintf(session,
